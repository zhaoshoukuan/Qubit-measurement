...
class Driver(BaseDriver):
    ...
    def demodulation(self,fft=False, avg=False, hilbert=False):
        e = self.config['e']
        n = e.shape[0]
        A_lst, B_lst = self.getData()
        if hilbert:
            Analysis = (signal.hilbert(A_lst),axis=0)).T
            A_lst, B_lst = np.real(Analysis), np.imag(Analysis)
        if fft:
            A_lst = (A_lst[:, :n]).dot(e)
            B_lst = (B_lst[:, :n]).dot(e)
    ...
    def getIQ(self, avg=False, hilbert=False):
        return self.demodulation(True, avg, hilbert)

    def getTraces(self, avg=True,hilbert=False):
        return self.demodulation(False, avg, hilbert)
