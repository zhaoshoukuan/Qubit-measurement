...
class GaussianDrag(Waveform):
    def __init__(self, width, alpha, Delta):
        """
        alpha: (0, 1]
        Delta: f21 - f10 - delta_f
        """
        super(Gaussian, self).__init__(domain=(-0.5*width,0.5*width))
        self.width = width
        c = self.width/(4*np.sqrt(2*np.log(2)))
...
class Exp(Waveform):
    def __init__(self, T):
        super(Exp, self).__init__(domain=(-np.inf, np.inf))
        self.timeFunc = lambda t: np.exp(-t/T)

class Expi(Waveform):
    def __init__(self, w, phi=0):
        super(Expi, self).__init__(domain=(-np.inf, np.inf))
        self.timeFunc = lambda t: np.exp(1j*(w*t+phi))

class CosPulse(Waveform):
    def __init__(self, width):
        super(CosPulse, self).__init__(domain=(-0.5*width,0.5*width))
        self.timeFunc = lambda t: (0.5*np.cos(2*np.pi*t/width)+0.5)*(t<0.5*width)*(t>-0.5*width)

class CosPulseDrag(Waveform):
    def __init__(self, width, alpha, Delta):
        """
        alpha: (0, 1]
        Delta: f21 - f10 - delta_f
        """
        super().__init__(domain=(-0.5 * width, 0.5 * width), outside=(0, 0))
        self.timeFunc = lambda t: (0.5 * np.cos(
            2 * np.pi * t / width) + 0.5 + 1j * alpha / (2 * Delta * width) *
                                   np.sin(2 * np.pi * t / width)) * (
                                       t < 0.5 * width) * (t > -0.5 * width)
...
__all__ = ['Waveform', 'DC', 'Interpolation', 'Step', 'Gaussian', 'Sin', 'Cos', 'Sinc', 'Exp', 'Expi', 'CosPulse', 'CosPulseDrag']
