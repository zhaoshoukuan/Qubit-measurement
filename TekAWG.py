...
class Driver(BaseDriver):
    ...
    def set_seq(self,
        ...
        """
        if tag == 'subseq':
            self.write('SLISt:SEQ:STEP%d:TASSet%d:SEQ "%s","%s"' %(step, track, seq_name, wave_name))
        else:
            for i, wav in enumerate(wave_name,start=2):
                ...
                if i == len(wave_name)+1:
                    if len(wave_name) % 64 !=0:
                        num = (len(wave_name)//64+1)*64 -len(wave_name) + 5
                    else:
                        num = 5
                    self.write('SLIS:SEQ:STEP%d:RCO "%s", %s' % (step, seq_name, num))
